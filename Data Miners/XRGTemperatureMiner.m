...
@property NSMutableDictionary<NSString *,NSMutableDictionary *> *sensorData;
...
- (void)updateCurrentTemperatures {
	...
	NSEnumerator *enumerator = [self.sensorData objectEnumerator];
	id value;
	...
	enumerator = [self.sensorData objectEnumerator];
	while (value = [enumerator nextObject]) {
...
- (NSString *)unitsForLocation:(NSString *)location {
	return self.sensorData[location][GSUnitsKey];
}
...
- (void)regenerateLocationKeyOrder {
	...
	for (typeIndex = 0; typeIndex < [types count]; typeIndex++) {
		...
		for (int i = 0; i < numLocations; i++) {
			...
			if (![self.sensorData[location][GSUnitsKey] isEqualToString:types[typeIndex]]) {
				continue;
			}

            NSString *humanReadableLocation = self.sensorData[location][GSLabelKey];
            if (!humanReadableLocation || [humanReadableLocation isEqualTo:self.sensorData[location][GSSensorRawKey]]) {
		...
		for (int i = 0; i < numLocations; i++) {
			if ((!alreadyUsed[i]) & [self.sensorData[locations[i]][GSUnitsKey] isEqualToString:types[typeIndex]]) {
...
- (float)currentValueForKey:(NSString *)locationKey {
	NSDictionary *tmpDictionary = self.sensorData[locationKey];
...
- (void)setCurrentValue:(float)value andUnits:(NSString *)units forLocation:(NSString *)location {
	...
	NSMutableDictionary *valueDictionary = self.sensorData[location];
	...
	if (valueDictionary == nil) {
        ...
        self.sensorData[location] = valueDictionary;
		needRegen = YES;
	}
	...
	if (valueDictionary[GSLabelKey] == nil) {
		if ([location isEqualToString:@"CPU A AD7417 AMB"]) {
			...
			valueDictionary[GSLabelKey] = [self.smcSensors humanReadableNameForKey:location];
		}
	}
				
    valueDictionary[GSSensorRawKey] = location;
...
- (NSString *)labelForKey:(NSString *)locationKey {
    id label = self.sensorData[locationKey][GSLabelKey];
...
- (void)setDataSize:(int)newNumSamples {
    ...
    for (int i = 0; i < [a count]; i++) {
		[self.sensorData[a[i]][GSDataSetKey] resize:(size_t)newNumSamples];
    }
